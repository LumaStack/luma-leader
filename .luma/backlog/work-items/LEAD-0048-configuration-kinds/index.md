...
> **Migrated from `.luma/backlog/ideas/` — read this first.** This was not an idea record. It arrived in the corpus as a `type: document` design document, and its body is reproduced verbatim below. **I am not sure this belongs in the backlog as a work item** — it may want to be a document under `docs/` instead, or it may want to be split into several ideas. It is here as an idea so that decision gets made rather than lost.
...
[[work-items/LEAD-0043-decision-ideas]] settled it on 2026-08-17 with the words *"two kinds of
...
- Born from the idea it replaces: [`configuration-kinds`](../../ideas/configuration-kinds.md)
