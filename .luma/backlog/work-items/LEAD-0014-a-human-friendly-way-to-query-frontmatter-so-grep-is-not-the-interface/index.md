...
description: 'We need a CLI way to run grep that is human friendly. It came out of arguing about whether created: { by, at } should have been created_by: and created_at:.'
...
# A human-friendly way to query frontmatter, so grep is not the interface
...
- *every document whose `stage` is unset* — **absence**, which grep
...
which is not the same as a field being unset, and it gets `stage:` in
...
[[work-items/LEAD-0005-three-frontmatter-parsers-no-reference-implementation-no-shared-tests]], which is the same problem viewed from the other
side.
...
- Born from the idea it replaces: [`a-human-friendly-way-to-query-frontmatter`](../../ideas/a-human-friendly-way-to-query-frontmatter.md)
